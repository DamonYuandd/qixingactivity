...
// 		$customDao = M('Custom');
    ...
    public function channel(){
        $zyChannel = C('ZY_CHANNEL');
        $yhChannel = C('YH_CHANNEL');
        $data = M('channel')->where(array('id' => 1))->find();
        
        $this->assign('data',$data);
        $this->assign('zyChannel',$zyChannel);
        $this->assign('yhChannel',$yhChannel);
        $this->display();
    }
    ...
    public function updateChannel(){
        ...
        $data = $_POST;
        $obj = M('Channel')->where(array('id' => 1))->save($data);
